...
class NeuraBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        for cog in self.cogs.values():
            if hasattr(cog, 'web_solver'):
                cog.web_solver = self.web_solver
            if hasattr(cog, 'captcha_solver'):
                cog.captcha_solver = self.captcha_solver
            if hasattr(cog, 'interactions'):
                cog.interactions = self.interactions

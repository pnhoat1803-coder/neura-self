...
def stats():
    ...
    session_cmds = st.get('session_hunt_count', 0) + st.get('session_battle_count', 0) + st.get('session_owo_count', 0) + st.get('session_other_count', 0)
    mins = elapsed / 60
    ...
    if len(history) > 1:
        ...
        if time_diff_hrs > 0.01: cph = round(cash_diff / time_diff_hrs)
    ...
    response_data = {
        ...
        'analytics': {'cph': cph, 'gems_used': st.get('gems_used', 0)},
        'bot': {
            'user_id': uid,
        ...
        'chart_data': {
            'hunt': st.get('hunt_count', 0), 'battle': st.get('battle_count', 0),
            'session_hunt': st.get('session_hunt_count', 0), 'session_battle': st.get('session_battle_count', 0),
            'session_owo': st.get('session_owo_count', 0), 'other': st.get('other_count', 0),
            'owo': st.get('owo_count', 0), 'total': st.get('total_cmd_count', 0), 'perf_bpm': cpm
        },
        'system': {'last_cash_update': st.get('last_cash_update', 0), 'pending_commands': len(st.get('pending_commands', []))},
    ...
    if request.method == 'POST':
        ...
        def restore_large_ints(obj):
            if isinstance(obj, dict):
                ...
                return [restore_large_ints(v) for v in obj]
            elif isinstance(obj, str) and obj.isdigit() and len(obj) >= 15:
                try: return int(obj)
                except ValueError: return obj
            return obj
            
        new_config = restore_large_ints(new_config)
...
def proxies_test():
    ...
    async def _run():
        if proxy_id:
            ...
            if not proxy: return {"ok": False, "error": "not found"}
...
def test_security():
    account_id = request.args.get('id')
    bot = get_bot(account_id)
    if not bot: return jsonify({'status': 'error', 'message': 'Bot not found'}), 404
...
def control():
    ...
    if action == 'stop':
        ...
        asyncio.run_coroutine_threadsafe(bot.send_message(f"{bot.prefix}cash", skip_typing=True, priority=True), bot.loop)
...
def captcha_current():
    ...
    if captcha_data and captcha_data.get('image_url'):
        ...
        if time.time() - timestamp < 600:
            return jsonify({'success': True, 'url': captcha_data['image_url'], 'cash': captcha_data.get('cash', 16000), 'command': captcha_data.get('command_template', 'owo autohunt {cash} {password}'), 'age_seconds': int(time.time() - timestamp)})
        else:
            if 'current_captcha' in st: del st['current_captcha']
...
def captcha_submit():
    ...
    bot = get_bot(account_id)
    if not bot: return jsonify({'success': False, 'error': 'Bot not found'})
    if not code: return jsonify({'success': False, 'error': 'No password provided'})
    st = bot.stats
    captcha_data = st.get('current_captcha')
    if not captcha_data: return jsonify({'success': False, 'error': 'No active captcha'})
    ...
    asyncio.run_coroutine_threadsafe(bot.send_message(full_command, skip_typing=True, priority=True), bot.loop)
    if 'current_captcha' in st: del st['current_captcha']
...
def captcha_balance():
    account_id = request.args.get('id')
    bot = get_bot(account_id)
    if not bot: return jsonify({'balance': None, 'service': 'unknown', 'error': 'Bot not found'})
    cfg = bot.config.get('security', {}).get('captcha_solver', {})
    service = cfg.get('service', 'yescaptcha')
    api_key = ''
    if request.method == 'POST':
        data = request.json or {}
        if 'service' in data: service = data['service']
        if 'api_key' in data: api_key = data['api_key']
    if not api_key:
        if service == 'nopecha': api_key = cfg.get('nopecha_api_key', cfg.get('api_key', ''))
        elif service == 'anticaptcha': api_key = cfg.get('anticaptcha_api_key', cfg.get('api_key', ''))
        elif service == 'captchaly': api_key = cfg.get('captchaly_api_key', cfg.get('api_key', ''))
        else: api_key = cfg.get('yescaptcha_api_key', cfg.get('api_key', ''))
    temp_solver = None
    if service == 'nopecha':
...
def captcha_stats():
    ...
    return jsonify({'solved': solved, 'success_rate': success_rate})
...
def bot_command():
    ...
    bot = get_bot(account_id)
    if not bot: return jsonify({'success': False, 'error': 'Bot not found'})
    if not command: return jsonify({'success': False, 'error': 'No command provided'})
    asyncio.run_coroutine_threadsafe(bot.send_message(command, skip_typing=True, priority=True), bot.loop)
...
def get_captcha_challenge():
    ...
    if account_id and account_id in _pending_captchas:
        return jsonify({'success': True, 'challenge': _pending_captchas[account_id]})
...
def submit_captcha_solution():
    ...
    if not account_id or not token: return jsonify({'success': False, 'error': 'Missing account_id or token'})
    bot = get_bot(account_id)
    if not bot: return jsonify({'success': False, 'error': 'Bot not found'})
    headers = {
    ...
    payload = {"token": token}
    try:
        response = requests.post("https://owobot.com/api/captcha/verify", json=payload, headers=headers, verify=False, timeout=10)
...
def captcha_oauth_url():
    data = request.get_json()
    account_id = data.get('account_id')
    if not account_id: return jsonify({'success': False, 'error': 'Missing account_id'})
    bot = get_bot(account_id)
    if not bot: return jsonify({'success': False, 'error': 'Bot not found'})
    import aiohttp
    import asyncio
    ...
    loop.close()
    if not redirect_url: return jsonify({'success': False, 'error': 'Failed to get OAuth URL'})
...
def pending_captchas():
    pending = []
    for acc_id, challenge in _pending_captchas.items():
        pending.append({'account_id': acc_id, 'account_name': challenge.get('account_name', acc_id), 'created_at': challenge.get('created_at', time.time())})
    return jsonify({'pending': pending})

def register_captcha_challenge(account_id, challenge_data):
    _pending_captchas[account_id] = {'account_id': account_id, 'created_at': time.time(), **challenge_data}

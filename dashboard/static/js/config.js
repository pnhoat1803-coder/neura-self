...
function renderCategoryFlat(obj, path, categoryName, depth = 0, parentEnabled = true) {
    if (path === 'commands.boss' || path === 'boss') {
        const selfEnabled = parentEnabled && (obj.enabled !== false || !('enabled' in obj));
        let h = '';

        h += `
            <div class="cfg-section ${depth > 0 ? 'cfg-section-nested' : ''}">
                <div class="cfg-section-head">GENERAL OPTIONS</div>
                <div class="cfg-section-rows">
                    ${renderField(`${path}.enabled`, { l: 'enabled', type: 'toggle', master: true }, obj.enabled ?? true, false, parentEnabled)}
                    ${renderField(`${path}.join_chance`, { l: 'join_chance' }, obj.join_chance ?? 100, false, selfEnabled)}
                </div>
            </div>
        `;
        
        h += `
            <div class="cfg-section ${depth > 0 ? 'cfg-section-nested' : ''} ${selfEnabled ? '' : 'cfg-section-disabled'}">
                <div class="cfg-section-head">GUILDS FILTER</div>
                <div class="cfg-section-rows">
                    ${renderField(`${path}.join_all_guilds`, { l: 'join_all_guilds', type: 'toggle' }, obj.join_all_guilds ?? false, false, selfEnabled)}
                    ${renderField(`${path}.ignore_all_guilds`, { l: 'ignore_all_guilds', type: 'toggle' }, obj.ignore_all_guilds ?? false, false, selfEnabled)}
                    ${renderField(`${path}.allowed_guilds`, { l: 'allowed_guilds', type: 'list' }, obj.allowed_guilds ?? [], false, selfEnabled)}
                    ${renderField(`${path}.ignore_guilds`, { l: 'ignore_guilds', type: 'list' }, obj.ignore_guilds ?? [], false, selfEnabled)}
                </div>
            </div>
        `;
        ...
        h += `
            <div class="cfg-section ${depth > 0 ? 'cfg-section-nested' : ''} ${selfEnabled ? '' : 'cfg-section-disabled'}">
                <div class="cfg-section-head">BOSS CHANNEL</div>
                <div class="cfg-section-rows">
                    ${renderField(`${path}.enable_boss_channel`, { l: 'enable_boss_channel', type: 'toggle' }, obj.enable_boss_channel ?? false, false, selfEnabled)}
                    ${renderField(`${path}.boss_channel`, { l: 'boss_channel', type: 'list' }, obj.boss_channel ?? [], false, selfEnabled)}
                </div>
            </div>
        `;
        ...
        h += `
            <div class="cfg-section ${depth > 0 ? 'cfg-section-nested' : ''} ${selfEnabled ? '' : 'cfg-section-disabled'}">
                <div class="cfg-section-head">ALL IN TICKET</div>
                <div class="cfg-section-rows">
                    ${renderField(`${path}.all_in_enabled`, { l: 'all_in_enabled', type: 'toggle' }, obj.all_in_enabled ?? false, false, selfEnabled)}
                    ${renderField(`${path}.all_in_guilds`, { l: 'all_in_guilds', type: 'list' }, obj.all_in_guilds ?? [], false, selfEnabled)}
                </div>
            </div>
        `;
        return h; 
...
function isListField(path) {
    ...
    return ['channels', 'targets', 'active_commands', 'ignore_guilds', 'allowed_guilds', 'boss_channel', 'boss_channels', 'all_in_guilds'].includes(name);
}
...
function toggleMod(p, el, ev) {
    ...
    if (p === 'commands.boss.join_all_guilds' && v === true) {
        setDeep(currentConfig, ['commands', 'boss', 'ignore_all_guilds'], false);
        requireReRender = true;
    } else if (p === 'commands.boss.ignore_all_guilds' && v === true) {
        setDeep(currentConfig, ['commands', 'boss', 'join_all_guilds'], false);
        requireReRender = true;
    }
    ...
    if (isGemTier) {
        ...
        if (p.endsWith('.enabled') || p.split('.').pop() === 'enabled' || requireReRender) {
            renderSettings(currentConfig);
        }
...
function updateDeepVal(p, v) {
    let val = v;
    const arrayFields = ['channels', 'targets', 'active_commands', 'ignore_guilds', 'allowed_guilds', 'boss_channel', 'boss_channels', 'all_in_guilds'];
